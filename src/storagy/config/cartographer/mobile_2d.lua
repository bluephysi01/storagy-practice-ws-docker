...
-- [드리프트/맵 늘어짐 방지]
-- 시뮬레이션 오도메트리(gz OdometryPublisher)는 모델의 실제 pose 로부터 계산되어
-- 사실상 ground-truth 다. 창고의 긴 복도/평탄한 벽에서는 진행 방향 스캔매칭이
-- 거의 구속되지 않으므로, 스캔매칭이 오도메트리 예측에서 멀리 벗어나지 못하도록
-- anchoring 가중치를 크게 올려 진행 방향으로 맵이 늘어지는 현상을 막는다.
